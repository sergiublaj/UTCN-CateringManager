...
/**
 * <p>Service class to update application state</p>
...
public class DeliveryService extends Observable implements IDeliveryServiceProcessing {
   ...
   /**
    * <p>Loads serialized information</p>
    */
   public void loadAll() {
      menuItems = ProductData.loadProducts();
   ...
   /**
    * <p>Imports the products from .csv to program</p>
   ...
   public boolean importProducts() {
      ...
      try {
         ...
         returnValue = ProductData.addProducts((HashSet<MenuItem>) menuItems);
      } catch (IOException e) {
         e.printStackTrace();
      }
      assert isWellFormed() : "[ERROR] importProducts - not well formed!";
   ...
   public boolean addProduct(MenuItem menuItem) {
      ...
      boolean returnValue = menuItems.add(menuItem) && ProductData.addProduct(menuItem);
      assert menuItems.contains(menuItem) : "[ERROR] addProduct - Menu item not present in menu list!";
      assert isWellFormed() : "[ERROR] addProduct - not well formed!";
      return returnValue;
   }
   ...
   public boolean modifyProduct(MenuItem menuItem) {
      ...
      boolean returnValue = menuItems.remove(menuItem) && menuItems.add(menuItem) && ProductData.modifyProduct(menuItem);
      assert menuItems.contains(menuItem) : "[ERROR] modifyProduct - Menu item not present in menu list!";
      assert isWellFormed() : "[ERROR] modifyProduct - not well formed!";
      return returnValue;
   }
   ...
   public boolean deleteProduct(MenuItem menuItem) {
      ...
      boolean returnValue = menuItems.remove(menuItem) && ProductData.removeProduct(menuItem);
      assert isWellFormed() : "[ERROR] deleteProduct - not well formed!";
      assert !menuItems.contains(menuItem) : "[ERROR] deleteProduct - Menu item not present in menu list!";
      return returnValue;
   }
   ...
   public boolean createProduct(MenuItem menuItem) {
      ...
      boolean returnValue = menuItems.add(menuItem) && ProductData.addProduct(menuItem);
      assert isWellFormed() : "[ERROR] createProduct - not well formed!";
      assert menuItems.contains(menuItem) : "[ERROR] createProduct - Menu item not present in menu list!";
      return returnValue;
   }
   ...
   public void generateTimeReport(int startHour, int endHour, JTextArea reportField) {
      assert 0 <= startHour && startHour < endHour && endHour < 24 : "[ERROR] generateTimeReport - Invalid order times!";
    ...
    */
   @Override
   public void generateRichReport(int orderedAmount, int orderValue, JTextArea reportField) {
      assert orderedAmount > 0 && orderValue > 0 : "[ERROR]: generateRichReport - Invalid amounts!";
   ...
   public boolean placeOrder(Order newOrder, Set<MenuItem> orderedProducts) {
      assert newOrder != null && !orderedProducts.isEmpty(): "[ERROR] placeOrder - Order does not contain any products!";
      ...
      if (returnValue) {
         ...
         assert orderList.containsKey(newOrder) : "[ERROR] placeOrder - Order not present in order list!";
      }
      return returnValue;
   }
   ...
   /**
    * <p>Checks if given list are the same</p>
